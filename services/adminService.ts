...
export let updateStatus: Function = async (param: tickets, body: tickets, token: tokenData): Promise<any> => {
        try {
            ...
            if (deleteStatus.rows[0].is_deleted) {
                return ({ status: RESPONSE_STATUS.FORBIDDEN, message: { error: "you are not allowed to update status of deleted ticket" } });
            }
            if (token.role == ROLE.USER) {
                return ({ status: RESPONSE_STATUS.UNAUTHORIZED, message: { error: "User not allowed to update status" } });
            }
            ...
            if (ticketStatus.rows[0].ticket_status == TICKET_STATUS.HOLD || ticketStatus.rows[0].ticket_status == TICKET_STATUS.PROGRESS) {
                return ({ status: RESPONSE_STATUS.FORBIDDEN, message: { error: "Admin can not set ticket status as hold or progress if ticket status already on hold or progress respectively " } });
            }
                ...
                return ({ status: RESPONSE_STATUS.FORBIDDEN, message: { error: "Admin can not change status of Approved and Rejected Ticket" } });
            }
            else {
                ...
                if (ticket_Info.rowCount != 0) {
                    return ({ status: RESPONSE_STATUS.SUCCESS, message: { succsses_message: "Ticket Status Updated Succssfully " } });
                }
            }
        }
        catch (err: unknown) {
